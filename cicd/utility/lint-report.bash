...
##	Distil warnings/advisories from the Go toolchain: go vet and go build write
##	plain "file:line: message" diagnostics; staticcheck tags each with a check id
##	(SAxxxx/STxxxx/Sxxxx/QFxxxx/Uxxxx); govulncheck reports GO-YYYY-NNNN advisories;
##	gosec (if wired) prints Gxxx rule ids + a Severity line. Drop the "0 findings"
##	noise a clean run emits.
warns="$(grep -inE 'warning|\b(SA|ST|S1|QF|U)[0-9]{3,4}\b|GO-[0-9]{4}-[0-9]+|vulnerab|\bG[0-9]{3}\b|Severity: (HIGH|MEDIUM)|^[^ ]+\.go:[0-9]+:[0-9]+:' "$log" 2>/dev/null \
	| grep -viE 'no problems found|0 (issues|findings|vulnerabilities|warnings)|no (known )?vulnerabilities|module verified|staticcheck: clean' || true)"
...
##	Script history:
##		- 20260709 JC: Created (adapted from the SilkTerm cicd for Go tooling).
